 ...
 *
 * chronoid_ksuid_string_batch dispatcher and scalar reference. The AVX2
 ...
 *   void chronoid_ksuid_string_batch(...) {
    ...
    size_t n)
{
  ...
  for (size_t i = 0; i < n; ++i)
    chronoid_ksuid_format (&ids[i], out_27n + i * CHRONOID_KSUID_STRING_LEN);
}
...
static _Atomic chronoid_ksuid_string_batch_fn g_batch_impl =
    &chronoid_ksuid_string_batch_init_trampoline;
    ...
    size_t n)
{
  ...
  if (n == 0)
    return;
  chronoid_ksuid_string_batch_fn f =
