 ...
 * across 8 KSUID lanes. The magic constant CHRONOID_KSUID_DIV62_M = floor(2^64
 * / 62) lives in chronoid/divisor_magic.h (auto-generated by
 ...
 *     chronoid_ksuid_mulhi64_avx2 below; mid_low is the sum of three u32
  ...
  for (size_t base = 0; base < bulk; base += 8) {
    ...
    for (int col = CHRONOID_KSUID_STRING_LEN - 1; col >= 0; --col) {
      ...
      for (size_t j = 0; j < 5; ++j) {
        ...
        chronoid_ksuid_divmod62_avx2 (v_lo, mag, &q_lo, &r_lo);
        chronoid_ksuid_divmod62_avx2 (v_hi, mag, &q_hi, &r_hi);
