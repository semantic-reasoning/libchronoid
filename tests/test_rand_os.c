...
test_returns_nonzero_bytes (void)
{
  ...
  ASSERT_EQ_INT (chronoid_os_random_bytes (buf, sizeof buf), 0);
...
{
  uint8_t a[32], b[32];
  ASSERT_EQ_INT (chronoid_os_random_bytes (a, sizeof a), 0);
  ASSERT_EQ_INT (chronoid_os_random_bytes (b, sizeof b), 0);
...
{
  uint8_t buf[1] = { 0xab };
  ASSERT_EQ_INT (chronoid_os_random_bytes (buf, 0), 0);
