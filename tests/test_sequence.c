...
static void
make_seed (chronoid_ksuid_t *seed)
{
...
{
  chronoid_ksuid_sequence_t s;
  chronoid_ksuid_t seed;
  make_seed (&seed);
  chronoid_ksuid_sequence_init (&s, &seed);

  chronoid_ksuid_t out;
  ASSERT_EQ_INT (chronoid_ksuid_sequence_next (&s, &out), CHRONOID_KSUID_OK);
  /* Leading 18 bytes preserved, last two are big-endian 0x0000. */
  ASSERT_EQ_BYTES (out.b, seed.b, CHRONOID_KSUID_BYTES - 2);
  ASSERT_EQ_INT (out.b[CHRONOID_KSUID_BYTES - 2], 0);
  ASSERT_EQ_INT (out.b[CHRONOID_KSUID_BYTES - 1], 0);
}
...
test_sequence_emits_big_endian_counter (void)
{
  ...
  chronoid_ksuid_t out;
  ASSERT_EQ_INT (chronoid_ksuid_sequence_next (&s, &out), CHRONOID_KSUID_OK);
  ASSERT_EQ_INT (out.b[CHRONOID_KSUID_BYTES - 2], 0x01);
  ASSERT_EQ_INT (out.b[CHRONOID_KSUID_BYTES - 1], 0x02);
...
test_sequence_emits_strictly_increasing (void)
{
  ...
  chronoid_ksuid_t prev, cur;
  ASSERT_EQ_INT (chronoid_ksuid_sequence_next (&s, &prev), CHRONOID_KSUID_OK);
  for (size_t i = 1; i < 1024; ++i) {
    ASSERT_EQ_INT (chronoid_ksuid_sequence_next (&s, &cur), CHRONOID_KSUID_OK);
    ASSERT_TRUE (chronoid_ksuid_compare (&prev, &cur) < 0);
    prev = cur;
  }
}
...
test_sequence_bounds_match_documentation (void)
{
  ...
  chronoid_ksuid_t lo, hi;
  /* Pre-iteration: min counter = 0, max counter = 0xFFFF. */
  chronoid_ksuid_sequence_bounds (&s, &lo, &hi);
  ASSERT_EQ_INT (lo.b[CHRONOID_KSUID_BYTES - 2], 0x00);
  ASSERT_EQ_INT (lo.b[CHRONOID_KSUID_BYTES - 1], 0x00);
  ASSERT_EQ_INT (hi.b[CHRONOID_KSUID_BYTES - 2], 0xff);
  ASSERT_EQ_INT (hi.b[CHRONOID_KSUID_BYTES - 1], 0xff);

  /* Post-exhaustion: bounds clamps min counter to 0xFFFF. */
  s.count = UINT16_MAX + 5u;
  chronoid_ksuid_sequence_bounds (&s, &lo, &hi);
  ASSERT_EQ_INT (lo.b[CHRONOID_KSUID_BYTES - 2], 0xff);
  ASSERT_EQ_INT (lo.b[CHRONOID_KSUID_BYTES - 1], 0xff);
  ASSERT_EQ_INT (chronoid_ksuid_compare (&lo, &hi), 0);
}
